  ...
  <div class="wiki-drafts">
    <div class="drafts-header">
      ...
      <p class="drafts-intro">
        Draft procedures are not visible to other users until you set status to Active. Edit to continue, or delete to
        remove.
      </p>
    </div>
    ...
    <ul v-else-if="drafts.length" class="drafts-list">
      <li v-for="p in drafts" :key="p.id" class="drafts-item">
        <router-link :to="{ name: 'ProcedureEditor', params: { slug: p.slug } }" class="drafts-link">
          {{ p.name }} (v{{ p.version }})
        </router-link>
        <span class="drafts-meta">Effective {{ p.effective_date }}</span>
        <div class="drafts-actions">
          <router-link :to="{ name: 'ProcedureEditor', params: { slug: p.slug } }" class="btn-edit" title="Edit draft">
            Edit
          </router-link>
    ...
    <p v-else class="drafts-empty">
      No draft procedures. Create one from the sidebar or when editing a procedure.
    </p>
...
import { getDraftProcedures, deleteProcedure } from '../api'
...
const drafts = ref<ProcedureSummary[]>([])
const loading = ref(true)
const error = ref('')
const deleting = ref(false)
const deleteTarget = ref<ProcedureSummary | null>(null)
...
onMounted(async () => {
  ...
  error.value = ''
  try {
    drafts.value = await getDraftProcedures()
  } catch (e) {
...
function confirmDelete(p: ProcedureSummary) {
...
async function handleDelete() {
  ...
  deleting.value = true
  try {
    await deleteProcedure(deleteTarget.value.id)
...
@use '../styles/views/procedure-drafts';
</style>
